...
app.get('/api/regulation/:commune', (req, res) => {
  ...
  // Construire le nom du fichier selon la nomenclature VS_[Commune]_Règlement des constructions.pdf
  const filename = `VS_${commune}_Règlement des constructions.pdf`;
  const filePath = path.join(__dirname, 'reglements', filename);
  ...
  if (fs.existsSync(filePath)) {
    ...
    res.status(404).json({ error: `Règlement non disponible pour la commune ${commune}` });
  }
});
